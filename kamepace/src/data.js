...
const fmtM = (m) => (m < 60 ? `${m}分` : (m % 60 ? `${Math.floor(m / 60)}時間${m % 60}分` : `${Math.floor(m / 60)}時間`));
const act = (id, glyph, icon, name, body, mind, defMin, extra = {}) => {
  const sum = body + mind;
  const recover = !!extra.recover;
  const est = Math.max(1, Math.round(sum * defMin / 60));
  return {
    id, glyph, icon, name, body, mind, defMin,
    fh: recover ? -sum : sum,
    last: `目安 ${recover ? '−' : '+'}${est}（${fmtM(defMin)}）`,
    ...extra,
  };
};
...
export const CATS = [
  { id: 'idou', icon: 'directions_subway', color: '#6f8fbf', glyph: '🚃', name: 'いどう', sub: '通勤通学・運転・送迎', items: [
    ...
    act('drive', '🚗', 'directions_car', '車の運転', 5, 7, 30, { kw: ['移動', '運転', '車', '渋滞'] }),
    act('pickup', '🚙', 'airport_shuttle', '送迎', 4, 5, 20, { kw: ['移動', '送り', 'お迎え', '子供'] }),
    act('walkmove', '🚶', 'directions_walk', '徒歩移動', 7, 2, 15, { kw: ['移動', '徒歩', '歩き'] }),
    act('bicycle', '🚲', 'directions_bike', '自転車', 8, 2, 15, { kw: ['移動', 'チャリ', 'サイクリング'] }),
  ] },
  { id: 'work', icon: 'work', color: '#c58b3d', glyph: '💼', name: '仕事', sub: '会議・デスクワーク・接客・現場', items: [
    act('meeting', '💬', 'forum', '会議・打ち合わせ', 3, 7, 60, { kw: ['仕事', '会議', 'ミーティング', 'MTG', '打ち合わせ'] }),
    act('docs', '📝', 'description', 'デスクワーク・資料作成', 4, 6, 60, { kw: ['仕事', '資料', '作業', '事務', 'パソコン'] }),
    act('workcall', '📞', 'call', '電話・通話対応', 2, 6, 15, { kw: ['仕事', '電話', '通話', 'コール'] }),
    act('reception', '🤝', 'support_agent', '来客・接客', 4, 8, 30, { kw: ['仕事', '接客', '来客', '窓口', 'クレーム'] }),
    act('fieldwork', '🧳', 'business_center', '外回り・出張', 7, 6, 120, { kw: ['仕事', '営業', '外回り', '出張'] }),
    act('labor', '🔧', 'construction', '力仕事・現場', 9, 4, 120, { kw: ['仕事', '現場', '力仕事', '肉体労働'] }),
    act('teach', '🧑‍🏫', 'cast_for_education', '教える・指導', 4, 7, 60, { kw: ['仕事', '指導', '研修', '教える'] }),
  ] },
  { id: 'baito', icon: 'storefront', color: '#d97a6a', glyph: '🏪', name: 'バイト', sub: '接客・レジ・調理・品出し', items: [
    act('serve', '🙋', 'support_agent', '接客', 5, 8, 180, { kw: ['仕事', 'バイト', '接客', 'ホール', 'クレーム'] }),
    act('register', '🧾', 'point_of_sale', 'レジ', 5, 6, 180, { kw: ['仕事', 'バイト', 'レジ', '会計'] }),
    act('cookwork', '🍳', 'restaurant', '調理', 7, 5, 180, { kw: ['仕事', 'バイト', '調理', 'キッチン'] }),
    act('stock', '📦', 'inventory', '品出し', 7, 3, 180, { kw: ['仕事', 'バイト', '品出し', '陳列'] }),
    act('carry', '🍽', 'room_service', '配膳', 8, 5, 180, { kw: ['仕事', 'バイト', '配膳', 'ホール'] }),
    act('cleanwork', '🧹', 'mop', '清掃', 7, 3, 180, { kw: ['仕事', 'バイト', '清掃', '掃除'] }),
  ...
  { id: 'study', icon: 'edit_note', color: '#8a7bc4', glyph: '📝', name: '課題・勉強', sub: 'レポート・テスト勉強・就活', items: [
    ...
    act('groupwork', '👥', 'group', 'グループワーク', 3, 7, 60, { kw: ['勉強', 'グループ', '共同', 'ゼミ'] }),
    act('jobhunt', '👔', 'badge', '就活・受験', 4, 9, 120, { kw: ['就活', '面接', 'ES', '受験'] }),
  ] },
  { id: 'club', icon: 'sports_soccer', color: '#58a34d', glyph: '⚽', name: '部活・サークル', sub: '練習・試合・ミーティング', items: [
    act('practice', '⚽', 'sports_soccer', '練習', 8, 4, 120, { kw: ['運動', '部活', 'サークル', '練習'] }),
    act('match', '🏆', 'emoji_events', '試合・大会・本番', 9, 8, 180, { kw: ['運動', '部活', '試合', '大会', '本番', 'ライブ'] }),
    act('clubmtg', '💬', 'forum', 'ミーティング', 2, 5, 30, { kw: ['部活', 'サークル', 'ミーティング'] }),
    act('clubadmin', '📋', 'assignment', '運営・幹部業務', 3, 7, 30, { kw: ['部活', 'サークル', '運営', '幹事'] }),
    act('camp', '🚌', 'directions_bus', '合宿・遠征', 8, 6, 480, { kw: ['部活', '合宿', '遠征', '旅行'] }),
  ] },
  { id: 'house', icon: 'cleaning_services', color: '#4fa88a', glyph: '🧹', name: '家事・生活', sub: '掃除・料理・育児・手続き', items: [
    act('dishes', '🍽', 'local_dining', '皿洗い', 5, 3, 15, { kw: ['家事', '皿洗い', '洗い物', '食器'], after: 1 }),
    act('cleanlaundry', '🧺', 'local_laundry_service', '掃除・洗濯', 6, 2, 30, { kw: ['家事', '掃除', '洗濯', '片付け'], after: 1 }),
    act('cook', '🍳', 'lunch_dining', '料理', 5, 3, 40, { kw: ['家事', '料理', '自炊', 'ごはん'] }),
    act('shop', '🛒', 'shopping_basket', '買い物', 5, 4, 30, { kw: ['家事', '買い物', '買い出し', 'スーパー'] }),
    act('childcare', '🍼', 'child_care', '育児', 7, 7, 60, { kw: ['家事', '育児', '子供', '保育'] }),
    act('carework', '🩺', 'medical_services', '介護・看病', 7, 8, 60, { kw: ['家事', '介護', '看病'] }),
    act('errands', '📄', 'receipt_long', '手続き・役所・病院', 4, 6, 60, { kw: ['手続き', '役所', '病院', '銀行', '待ち時間'] }),
  ] },
  { id: 'talk', icon: 'groups', color: '#b07bc4', glyph: '💬', name: '対人', sub: '相談・飲み会・イベント', items: [
    act('consult', '💬', 'groups', '打ち合わせ・相談', 3, 7, 30, { kw: ['対人', '相談', '打ち合わせ'] }),
    act('privatecall', '📞', 'call', '電話・通話', 2, 6, 20, { kw: ['対人', '電話', '通話'] }),
    act('party', '🍻', 'local_bar', '交流会・飲み会', 5, 7, 120, { kw: ['対人', '飲み会', '交流会', '宴会', 'イベント'] }),
    act('hangout', '🎉', 'celebration', '遊び・イベント', 5, 5, 120, { kw: ['対人', '遊び', 'イベント', 'お出かけ'] }),
  ...
  { id: 'rest', icon: 'self_improvement', color: '#7a9a00', glyph: '☕', name: '休憩・回復', sub: 'お茶・入浴・昼寝・趣味 ・ −回復', items: [
    ...
    act('bath', '🛁', 'bathtub', '入浴', 5, 7, 20, { recover: true, kw: ['休憩', '風呂', 'お風呂', 'サウナ'] }),
    act('nap', '😴', 'bedtime', '昼寝・仮眠', 7, 6, 20, { recover: true, kw: ['休憩', '昼寝', '仮眠', '寝る'] }),
...
const SEARCH_VARIANTS = [
  { name: '通勤（電車・座れた）', glyph: '🚃', fh: 7, body: 4, mind: 3, defMin: 30, kw: ['通勤', '電車', '移動'] },
  { name: '通勤（電車・立ち）', glyph: '🚃', fh: 10, body: 6, mind: 4, defMin: 30, kw: ['通勤', '電車', '移動'] },
  { name: '通勤（バス）', glyph: '🚌', fh: 8, body: 4, mind: 4, defMin: 30, kw: ['通勤', 'バス', '移動'] },
  { name: 'オンライン会議', glyph: '💻', fh: 8, body: 2, mind: 6, defMin: 60, kw: ['会議', 'オンライン', '仕事'] },
...
export const SEARCH_DB = [
  ...
  ...CATS.flatMap(c => c.items.map(it => ({
    name: it.name, glyph: it.glyph, fh: it.fh, body: it.body, mind: it.mind, defMin: it.defMin,
